...
def load_obs(phottable='demo_photometry.dat', **kwargs):
  ...
  corrs = correct_ext(filternames) * (Av/0.73)
  # !!!!!!!!!!!!
...
def correct_ext(filters, zred=0, exttype='MW'):
...
model_params.append({'name': 'mass', 'N': 1,
                        'isfree': True,
                        'init': init_mass,
                        'init_disp': init_disp_mass,
                        'units': r'M_\odot',
                        #'prior':priors.TopHat(mini=1e5, maxi=1e11)})
                        'prior':priors.LogUniform(mini=1e5, maxi=1e11)})
...
def load_model(**extras):
    ...
    # FSPS parameter
    # making age dependent on redshift
    age = cosmo.age(zred).value # age in Gyr, determines prior and init
    model_params.append({'name': 'tage', 'N': 1,
                            'isfree': True,
                            'init': age/2.,
                            'init_disp': age/2.,
                            'units': 'Gyr',
                            'prior':priors.LogUniform(mini=0.0001,maxi=cosmo.age(zred).value)})
    """Original age
    model_params.append({'name': 'tage', 'N': 1,
                            'isfree': True,
                            'init': init_tage,
                            'init_disp': init_disp_tage,
                            'units': 'Gyr',
                            'prior':priors.LogUniform(mini=0.0001,maxi=cosmo.age(zred).value)})
    """
    # FSPS parameter
