...
# Create dependencies, please work...
def burst_end(sf_start, **extras):
  return sf_start+.010

def age_rectifier(tage, sf_start, **extras):
  if sf_start > tage:
    return sf_start + 0.010
  else:
    return tage

def mass_frac(fburst, const, **extras):
  if fburst + const > 1:
    return (1-const)
  else:
    return fburst

def tau_fixer(tage, **extras):
  return -tage*1j/(2*np.pi)
# --------------
# SPS Object
# --------------
...
model_params.append({'name': 'tau', 'N': 1,
                        'isfree': False,
                        #'depends_on': tau_fixer,
                        'init': np.inf,
                        'units': 'Gyr',
                        'prior':priors.LogUniform(mini=0.005, maxi=100)})
...
def load_model(**extras):
    ...
    model_params.append({'name': 'fburst', 'N': 1,
                        ...
                        'init': 0.0,
                        'units': '',
                        'depends_on': mass_frac,
                        'prior':priors.TopHat(mini=0.0, maxi=1.0)})

    # FSPS parameter
    model_params.append({'name': 'const', 'N': 1,
                        'isfree': True,
                        'init': 0.0,
                        #'depends_on': mass_frac,
                        'units': '',
